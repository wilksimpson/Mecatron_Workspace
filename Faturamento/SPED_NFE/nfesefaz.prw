					...
					*/
					...
					If lContinua
						...
						If lNfCup .Or. (cAliasSD2)->D2_ORIGLAN $ "VD|LO"
							...
							// Custumizações Mecatron Tipo Frete puxando do Loja
							IF EMPTY(SF2->F2_TPFRETE)
								If SL1->L1_TPFRET == "C"
									cModFrete := "0"
								ElseIf SL1->L1_TPFRET == "F"
						 			cModFrete := "1"
								ElseIf SL1->L1_TPFRET == "T"
						 			cModFrete := "2"
								ElseIf SL1->L1_TPFRET == "R"
					 				cModFrete := "3"
								ElseIf SL1->L1_TPFRET == "D"
					 				cModFrete := "4"
								ElseIf SL1->L1_TPFRET == "S" .or. SL1->L1_TPFRET == "0"
						 			cModFrete := "9"
						 		Else
						 			cModFrete := "1" 			 	 	
								EndIf   			 
					...
					If lNfCup .Or. (cAliasSD2)->D2_ORIGLAN $ "VD|LO"
						...
						If lAchouSL1
						    ...
						    // CUSTOMIZAÇÕES MECATRON - ADICONAR INFORMAÇÕES NO APEDCOM PARA COMPRA XPED
							IF  LEN(aPedCom)>0 .AND. !EMPTY(SL1->L1_PEDCLI)
								SL2->(DbSetOrder(1)) //L2_FILIAL+L2_SERIE+L2_DOC+L2_PRODUTO
								If SL2->(DbSeek(xFilial('SL2') + SL1->L1_NUM + (cAliasSD2)->D2_ITEMPV+(cAliasSD2)->D2_COD ))
									//aadd(aPedCom,{SC6->C6_NUMPCOM,SC6->C6_ITEMPC})
									IF !EMPTY(SL2->L2_ITPEDCL)
										aPedCom[LEN(aPedCom)] := {alltrim(SL1->L1_PEDCLI),alltrim(SL2->L2_ITPEDCL)}
									ELSE
										aPedCom[LEN(aPedCom)] := {alltrim(SL1->L1_PEDCLI),SL2->L2_ITEM}
									ENDIF
...
// Customizações Mecatron xPed
//If !empty(aProd[38]) .And. !empty(aProd[39])
//	cString += '<xPed>'+ConvType(aProd[38])+'</xPed>'/
//	cString += '<nItemPed>'+ConvType(aProd[39])+'</nItemPed>'
//Endif
